...
const finalPrompt =
  normalizedType === "medical"
    ? systemPrompt + " 医療画像の場合でも、まず自然な一言から入る。その後、見た目から考えられる状態を1つか2つに絞って短く説明する（なぜそう見えるかも一言入れる）。次に今できる具体的な対処を1つ出す。余裕があれば軽く補足を1つだけ足す。受診の話は必要な時だけ最後に軽く触れる。全体は2〜4文、LINEの会話のように短くするが、中身は薄くしない。内部では通常のChatGPTと同じレベルで分析してから答えること。処方箋や薬剤の画像では、薬の名前・用途・よくある注意点を優先して短く説明する。"
    : normalizedType === "food"
