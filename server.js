...
const finalPrompt =
  normalizedType === "medical"
    ? systemPrompt + " 医療画像の場合でも、まず自然な一言から入る。その後、見た目から考えられる状態を1つか2つに絞って短く説明する（なぜそう見えるかも一言入れる）。次に今できる具体的な対処を1つ出す。余裕があれば軽く補足を1つだけ足す。受診の話は必要な時だけ最後に軽く触れる。全体は2〜4文、LINEの会話のように基本は短くするが、内容が重要な場合（薬、検査結果、リスクがある場合）は必要な分だけ説明を増やしてよい。中身は薄くしない。内部では通常のChatGPTと同じレベルで分析してから答えること。処方箋や薬剤の画像では、薬の名前・用途・よくある注意点を優先して短く説明する。"
    : normalizedType === "food"
    ? systemPrompt + " 食べ物の画像の場合は、味や状態に一言触れて、より良く食べる工夫や楽しみ方を1つだけ短く返す。説明しすぎない。"
    : normalizedType === "document"
    ? systemPrompt + " ? systemPrompt + " 検査結果・処方箋・薬剤情報などの文書画像では、まず画像内の文字をできるだけ読んで、薬剤名や検査項目名を具体的に拾う。処方箋や薬の画像なら、最初に読めた薬の名前を箇条書きではなく自然な文で1つずつ挙げ、そのあと各薬について主な用途を短く説明する。読みに自信がない薬は『〜に見える』と添えてよいが、名前をなるべく出す。一般論だけで済ませない。最後に、全体としての注意点を1つだけ短く添える。"、まず何の画像かを一言で示す。その後、重要な内容を2点まで優先して短く説明する。処方箋や薬の画像なら、薬の名前、主な用途、よくある注意点を優先して述べる。検査結果なら、重要な異常を優先し、その意味を短く説明する。最後に今できることや確認すべきことを1つだけ具体的に出す。長文にしないが、内容は通常のChatGPT並みに薄くしない。受診の話は必要時だけ軽く添える。"
    : systemPrompt;
