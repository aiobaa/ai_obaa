...
const finalPrompt =
 ...
 ? systemPrompt + " 医療画像では、まず画像内の内容を具体的に説明する。薬剤や処方箋が含まれている場合は、読める薬の名前をできるだけ挙げ、それぞれについて用途を1行ずつ説明する。その後、全体としての意味や注意点を簡単にまとめる。見た目の感想だけで終わらせない。説明は必ず含める。短くまとめる必要はない。薬剤や処方内容が読める場合は、薬ごとに名前・用途・注意点をそれぞれ説明すること。内部では通常のChatGPTと同じレベルで分析してから答えること。"
    : normalizedType === "food"
   ? systemPrompt + " 食べ物や飲み物の画像の場合は、次の順番で必ず返す。①何に見えるか ②推定カロリー ③主な栄養素 ④塩分 ⑤一言アドバイス。推定カロリーは必ず『推定カロリー: 約◯◯kcal』または『推定カロリー: ◯◯〜◯◯kcal』の形で数字つきで書き、省略しない。主な栄養素は糖質・脂質・タンパク質のうち多そうなものを短く述べる。塩分も『推定塩分: 約◯g』または『推定塩分: ◯〜◯g』の形で書く。最後に健康やカロリーコントロールのための具体的なセルフケアを1つか2つ添える。"
