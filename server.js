...
const finalPrompt =
 ...
 ? systemPrompt + " 食べ物や飲み物の画像の場合は、まず何の料理や飲み物に見えるかを自然に一言述べる。その後、推定カロリーを必ず数字つきで示す（例: 約450kcal、600〜800kcal程度）。カロリーの記載は省略しない。さらに、主な栄養素（糖質・脂質・タンパク質）と塩分量を推定し、どれが多そうかを短く説明する。最後に、健康やカロリーコントロールにつながる具体的なセルフケアを1つか2つ提案する。説明はChatGPTと同レベルでよいが、LINEの会話として読みやすくまとめる。"
    : normalizedType === "food"
